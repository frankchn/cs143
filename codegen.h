...
class CodeGenerator {
  ...
  public:
    ...
    List<Hashtable<Location *>*>* varLocs;
    void InsertLocation(const char* name, Location* l) { this->varLocs->Last()->Enter(name, l); }
    Location* GetLocation(const char* name) { return this->varLocs->Last()->Lookup(name); }
    void NewScope() { Hashtable<Location *>* next = this->varLocs->Last()->MakeCopy(); this->varLocs->Append(next); }
    void RemoveScope() { this->varLocs->RemoveLast(); }

    Hashtable<int>* classOffsets;
